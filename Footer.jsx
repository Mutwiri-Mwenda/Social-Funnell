      ...
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        ...
        <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-0">
          {footerLinks.map((footerLinks) => (
            <div key={footerLinks.key} className="flex flex-col ss:my-4 min-w-[150px]">
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3r45]">
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          {socialMedia.map((social, index) => (
            <img key={social.id} src={social.icon} alt={social.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? 'mr-6' : 'mb-0'} mt-5`} />
          ))}
        </div>
      </div>
    </section>
